...
fastq_df <- fastq_df %>% mutate(seq_list=str_split(sequence,pattern=""))
...
count_N(fastq_seq_list[[2]])
fastq_df <- fastq_df %>% mutate(num_N=map_int(seq_list,count_N))

ggplot(fastq_df,aes(x=num_N)) + geom_histogram()
```
